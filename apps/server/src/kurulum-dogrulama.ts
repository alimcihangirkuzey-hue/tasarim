...
export interface KurulumIlani {
  ...
  env: readonly string[];
  /**
   * İlanın okuyucusunu barındıran kaynak dosya (server/src'e göreli).
   *
   * NİYE İLANIN KENDİ BEYANI (2026-08-07, kurulum kapıları paketi): kapı
   * nöbetçisinin "bu ilanın kaç RED YOLU var" sorusunu cevaplaması için
   * kaynağı okuması gerekir. Ad→dosya eşlemesini testin içinde kurmak, `env`
   * alanının kaldırdığı hatanın aynısını geri getirirdi: eşleme unutulabilir,
   * alan unutulamaz (tip zorunlu kılar). Bugün ad ile dosya adı örtüşüyor ama
   * bu bir ÂDETTİR; âdete dayanan bir nöbetçi, âdeti bozan ilanı görmez.
   */
  kaynak: string;
  oku: () => unknown;
}
...
export const KURULUM_ILANLARI: ReadonlyArray<KurulumIlani> = [
  { ad: "is-kollari", env: [IS_KOLLARI_ENV], kaynak: "is-kollari.ts", oku: isKollariIlani },
  { ad: "kurulum-kunyesi", env: [KUNYE_ENV], kaynak: "kurulum-kunyesi.ts", oku: kurulumKunyesi },
  ...
  { ad: "veri-dizini", env: [VERI_DIZINI_ENV], kaynak: "veri-dizini.ts", oku: () => veriDiziniIlani(path.join(ROOT_DIR, "data")) },
  ...
  { ad: "dogus-varsayilanlari", env: [PARA_BIRIMI_ENV, CIKTI_DILI_ENV], kaynak: "dogus-varsayilanlari.ts", oku: dogusVarsayilanlari },
  ...
  { ad: "acilis-takimi", env: [ACILIS_TAKIMI_ENV], kaynak: "acilis-takimi.ts", oku: () => { dogrulaTakimKapsami(); } },
];
