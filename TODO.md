...
### PAKET-F7-A — temel veri modeli TAMAMLANDI (2026-07-10, dal `paket-f7-a`, merge onayı bekliyor)
- [x] **F7-A teslim:** Item.ingredients[] (IngredientRef, D1 inline denormalize + ≥1 dil refine) · Client.menu_language (fr/de) · sector.ts Zod iskeleti (İÇERİK YOK) · migration v8 (ingredient_library + menu_language kolonu) · projeksiyon (intake→katalog: varyant→prices, çip→ingredients+desc_fr, ek-ikram→desc_fr " · " ayraç, boş fiyat→pending) · K3 fiyatsız=al+işaretle + empty-price analiz uyarısı · desc_fr backfill yardımcısı · POST /api/clients opsiyonel currency+menu_language. Paket yeşil (291); canlı Aras/Arriva **SALT-OKUMA** geriye-uyumlu parse doğrulandı (ingredients:[] eklenir, mevcut alanlar aynen; DB dokunulmadı).
- [ ] **F7-B (SIRADAKİ) — sektör paketi İÇERİKLERİ:** kebap/pizza/lokanta/café/pastane hazır kategori+ürün+soru+çip setleri. sector.ts iskeleti hazır; İÇERİK **kullanıcı onayı bekliyor** (tohum sızması yasak). Bağlı işler: intake UI (tıkla-seç akış, mobil), ingredient_library CRUD + öğrenme route'u, tam çok-dilli render (desc bugün menu_language'a göre kuruluyor ama şablonlar hâlâ desc_fr'den basıyor).
