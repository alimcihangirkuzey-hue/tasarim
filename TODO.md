...
- [ ] **F7-C GÖRSEL KABUL BORCU (kullanıcı manuel QA — main'de, start-tezgah.bat, özel kurulum yok — artık HF1 ile launcher güvenilir):** akışı uçtan uca sür (müşteri→paket→ürün→sorular/çipler→çeklist→commit) + **A4 önizleme** (sonuç ekranı "Menü önizle") + **~390px mobil** görünüm + **özet pending/gaps** kontrolü. Fonksiyonel zincir E2E ile kanıtlı (14/14); yalnız görsel doğrulama ertelendi (Chrome eklentisi otonom oturumda hâlâ bağlı değil). Bulgu çıkarsa **hotfix paketi**.
