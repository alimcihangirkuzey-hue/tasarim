...
- [ ] **F7-B2 (SIRADAKİ) — sektör paketi + çip TOHUM İÇERİKLERİ:** kebap/pizza/lokanta/café/pastane setleri (`SECTOR_PACKS` + `INGREDIENT_SEED` saf veri). **Kullanıcı onayı bekliyor** (tohum sızması yasak). Zemin hazır: şema (sector.ts) + servis (F7-B1) + merge.
- [ ] **F7-C — intake UI** (tıkla-seç akış, mobil-öncelikli) + intake-commit ucu: projeksiyonu kataloğa yazar VE **usage_count'u burada bump'lar** (kullanılan çip id'leri; seed ise source=seed satırı upsert). B1'de usage_count yalnız OKUNUR (bump YOK — M10, tüketicisiz uç yazılmadı). + tam çok-dilli render (desc bugün menu_language'a göre kuruluyor ama şablonlar hâlâ desc_fr'den basıyor).
