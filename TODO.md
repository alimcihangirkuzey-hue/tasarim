...
- [ ] **F7-C — intake UI** (tıkla-seç akış, mobil-öncelikli) + intake-commit ucu: projeksiyonu kataloğa yazar VE **usage_count'u burada bump'lar** (kullanılan çip id'leri; seed ise source=seed satırı upsert). B1'de usage_count yalnız OKUNUR. **Çip düzenleme + "içerik basılmasın" anahtarı** default_chips DOLU her item'da YERLEŞİK (q_icerik'in yerini alır — #3 kararı); schema'ya `chips` affects'i gerekirse o gün. + tam çok-dilli render (desc menu_language'a göre kuruluyor ama şablonlar hâlâ desc_fr'den basıyor).
- [ ] **SectorPackCategory'de kategori NOTU alanı YOK (F7-C/schema adayı):** F7-B2'de "Tabaklar: frites+salade ile servis", "Günün Yemekleri: her gün değişir" gibi kategori notları schema'da yer olmadığından TAŞINMADI. Gerekirse `SectorPackCategory.note` eklenir (schema evrimi).
