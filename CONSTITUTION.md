...
- Faz 6 / FAZ6-GOREV.md — mimar kararı #19: Fabrika SVG bekçisi (gerçek-dünya InDesign/Illustrator çıktısı). (a) Fiziksel ölçü SVG'den türetilemiyorsa (birimsiz width/height, yalnız viewBox) içe almada kullanıcıdan gerçek en×boy (mm) istenir; şablonun doğal ölçüsü buna kilitlenir. *Uygulama notu (#19a, mimar onayı 2026-07-09): TÜRET-ÖNCE-SOR — `deriveSizeMm` root width/height birimini (mm/cm/in/px) çözebiliyorsa otomatik; çözemezse sorar. Faz 4'teki A4-genişlik varsayımı KALDIRILDI.* (b) Eğriye çevrilmiş (outline) metin kabul edilir ama slot bağlanamaz — fabrika tespit eder, "dekor kalır; canlı slot için metni canlı bırakıp fontu yükleyin" uyarısı verir. (c) Canlı metnin font-family'si sistemde yoksa (yerleşik 6 + custom_fonts) uyarı + F5 font yükleme akışına yönlendirme; font yüklenmeden o metne slot bağlanması ENGELLENİR (M3). (d) Harici bağlantılı raster (file:///, http) render'a girmez → "eksik varlık" listelenir; gömülü (data:) kabul; toplam SVG üst sınırı 25MB (aşarsa RED). (e) script/foreignObject/dış-referanslı style temizlenir — *mevcut `factory/sanitize.ts` bunu zaten yapıyor (script/foreignObject/on*/javascript:/harici href/@import), teyit edildi.*
- Faz 6 / FAZ6-GOREV.md — mimar kararı #20 (REVİZE): Fabrika şablon KÜNYESİ (provenance): kaynak dosya adı, kullanıcı kaynak yol/not, tespit edilen font aileleri, gömülü/eksik varlık listesi, `svg_sha256`, içe alma tarihi. Amaç: yıllar sonra dönen işte sıfır arkeoloji; UI'da künye kartı. *Revizyon (mimar onayı 2026-07-09): #20'nin ilk hali "fabrika şablon tablosuna kolonlar + migration v8" diyordu; mimar #12 ile çelişki (fabrika bir KOD üreticisidir, DB'de fabrika şablon tablosu YOKTUR) Opus tarafından yakalandı. Künye, üretilen `manifest.ts` içinde `provenance` alanı olarak yaşar (`TemplateManifest.provenance?`, `FactoryInput` üzerinden dolar); kayıt defterinden okunur, API döndürür. **Migration v8 İPTAL — Faz 6 migrationsız fazdır.***
- Faz 6 / FAZ6-GOREV.md — mimar kararı #21: Serbest ölçü YALNIZ fabrika hattında. Fabrika şablonu SVG'nin gerçek ölçüsüyle doğar (standart formata eşlenmez); belge o ölçüde çalışır; export bleed 3mm + crop marks ile custom sayfa üretir (sayfa = (w+6)×(h+6)mm). Yerleşik kurallı şablonlar custom format ALMAZ (M8 garantileri standart formatlara göredir). Sınırlar 30×30mm – 3000×3000mm. Sıfır-slotlu (salt dekor) fabrika şablonu GEÇERLİDİR — cam/folyo gibi işlerde TEZGÂH ölçü+çıktı+künye üstlenir, kompozisyon dış araçta kalır (M2). *Uygulama notu (mimar onayı 2026-07-09): önceki `validateFactoryInput` "en az bir slot" kuralı bilinçli KALDIRILDI (dekor/cam-folyo hattı için sıfır-slot serbest). Belge tarafı custom ölçü `params.width_mm/height_mm` ile Zod-only — migrationsız.*
