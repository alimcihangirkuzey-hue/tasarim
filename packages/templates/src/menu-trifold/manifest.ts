...
export const manifest = {
  ...
  params: [
    ...
    { id: "designSeed", type: "number", default: 0, min: 0, max: 9999, step: 1, label_tr: "Tasarım tohumu (0 = taban)" },
    {
      id: "qrSource",
      type: "choice",
      options: ["review", "tel", "delivery", "instagram", "menu", "web"], // mimar #16: dijital menü adresi; "web": journal 2026-07-28-web-alani (F5-11 dersi: options açılmazsa seçilemez)
      default: "review",
      label_tr: "QR kaynağı",
    },
  ],
  slots: [
    ...
    /* boş kalabilir (gereklilik ilânı yok — ölü `optional` alanı kaldırıldı).
       Dosya ROLÜ İLANI (7.2/502): bind:null → rol TÜRETİLEMEZ, kabul ELLE
       yazılır; kapak FOTOĞRAFIDIR, marka logosu değil ("logo" bilerek dışarıda). */
    { id: "cover_photo", kind: "image", bind: null, kabul: ["photo", "other"] },
