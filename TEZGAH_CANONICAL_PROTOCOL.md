...
> **Şerh güncellemesi (2026-07-20):** Cockpit sonrası **C mi D mi** sorusu kapandı — **C öncelikli** (ürün sahibi kararı; bkz. 11.8 tablosu ve EK-B / K-22). Sıra: A→B→**C**→D.
...
| K-04 | **ADR/TDR ayrımı** kuruldu; mimari ve teknoloji kararları ayrı numaralandırılır. ✅ **Şerh (v4.2.0):** çakışma **icra edildi** — §0.3 eşleme şerhi + canlı atıf nitelemesi; arşiv korundu, yeniden adlandırma yapılmadı (bkz. EK-B / K-21). |
...
**Sürüm 4.2.0'da alınan kararlar**

| # | Karar |
|---|---|
| K-21 | **A-08 (ADR numaralandırma çakışması) icra edildi ve KAPANDI.** Çözüm: arşiv **yeniden adlandırılmadı**, tarihsel kayıt korundu; çakışma §0.3 **numaralandırma eşleme şerhiyle** (üç seri: canlı program `docs/adr/ADR-00n` · arşiv teknoloji `CONSTITUTION ADR-1..8` · ileri `TDR-nnn`) ve canlı atıfların açık nitelenmesiyle giderildi. **Ölçülen kalan çıplak atıf envanteri — 9 dosya:** `editorStore.ts:1` (ADR-6) · `cmyk.ts:1` + `api.ts:244` + `tr.json:257` + `EditorPage.tsx:503` + `export-kinds.md:18` (ADR-4) · `main.tsx:28` + `fonts.css:2` (ADR-3) · `fonts.ts:23` (ADR-8). §0.3 eşleme şerhi **hepsini** kanonik olarak disambigüe eder; dosya-içi yorum nitelemesi (UI + docs, davranış-dışı) ayrı bir mekanik tura devredildi (`TODO.md`). C-P0 Adım A'da yalnız `assets.ts` + `TODO.md` nitelendi (kapsam darlığı). |
| K-22 | **Cockpit sonrası ürün sırası C öncelikli** (2026-07-20 ürün sahibi kararı): C (Production Profiles) atlanmayacak; sıra Canonical 10.1'e göre A→B→**C**→D. "Öncelikli SaaS paketleri" ibaresi D'yi C'den **sonraya** koyar, C'yi atlamaz. İlk C paketi: C-P0 Üretim Profili Kimlik Katmanı (ilk profil: flyer). |
...
*A-08 (ADR numaralandırma çakışması) karara bağlandı ve bu tablodan düşürüldü — bkz. EK-B / K-21 (2026-07-20).*
