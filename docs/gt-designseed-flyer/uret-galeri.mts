/* designSeed@flyer GÖRSEL GT galerisi (Canonical 4.4; journal 2026-07-26-designseed-flyer).
   Çalıştırma (repo kökünden — JSX automatic runtime için tsconfig ŞART):
     npx tsx --tsconfig packages/templates/tsconfig.json docs/gt-designseed-flyer/uret-galeri.mts

   İçerik: (1) beş çerçeve varyantının her biri — temsilci tohumla, taban dahil;
   (2) tek tohumun üç temada tutarlılığı (çerçeve dili temadan bağımsız).
   Tohum temsilcileri deterministik taranır (ilk-görülen) — elle seçim yok. */
...
import { TEMPLATES, analyzeFlyer, type FlyerFrame } from "@tezgah/templates";

const HERE = path.dirname(fileURLToPath(import.meta.url));
const ROOT = path.resolve(HERE, "..", "..");
const CIKTI = path.join(HERE, "cikti");

/* ── Sabit müşteri (deterministik) ─────────────────────────────────────── */

const ADLAR = ["Kebab", "Pizza Margherita", "Tacos XL", "Burger Maison", "Salade César", "Pide Fromage", "Lahmacun", "Soupe du Jour", "Baklava"];
...
function musteri(): ClientDTO {
  return {
    ...
    brandkit: BrandKitSchema.parse({
      slogan_fr: "Le goût authentique",
      contact: { phone: "04 78 00 00 00" },
    }),
...
const doc = (params: Record<string, unknown> = {}, theme_id?: string): DocumentState =>
  DocumentStateSchema.parse({ template_id: "flyer", params, theme_id });

function svgOf(d: DocumentState, c: ClientDTO): string {
  return renderToStaticMarkup(
    React.createElement(TEMPLATES["flyer"].Component, { client: c, doc: d, mode: "print", pageIndex: 0 })
  );
}
...
const c = musteri();
const temsilci = new Map<string, { seed: number; frame: FlyerFrame }>();
for (let s = 1; s <= 200 && temsilci.size < 5; s++) {
  const frame = analyzeFlyer(c, doc({ designSeed: s })).frame;
...
const frameEtiket = (f: FlyerFrame): string =>
  `panel rx ${f.campaignRx} · hücre rx ${f.cellRx} · çizgi ${f.cellDash === null ? "düz" : `kesikli ${f.cellDash}`}`;
...
const varyantKartlari: Kart[] = [
  {
    ...
    alt: frameEtiket(analyzeFlyer(c, doc({})).frame),
    svg: svgOf(doc({}), c),
  },
...
<title>designSeed@flyer — çerçeve dili varyantları (görsel GT)</title>
<style>
${fontCss()}
...
<h1>designSeed@flyer — çerçeve dili varyantları (Canonical 4.4)</h1>
<div class="protokol">
<p><strong>Yargı kalemleri (insan):</strong> çerçeve dili TUTARLI mı (kampanya paneli ile
mini hücreler aynı dili konuşuyor mu)? · beş varyantın her biri baskıya çıkacak kalitede
ve marka-nötr mü? · taban (seed 0) gözle de bozulmamış mı?</p>
<p><strong>Makinece zaten doğrulananlar</strong> (testli): taban birebir bugünkü çıktı ·
tohum YALNIZ çerçeveyi değiştirir (kapasite/yerleşim/uyarılar derin-eşit) · kapalı 5'li
küme dışına çıkılamaz · aynı tohum daima aynı çerçeve.</p>
<p><strong>Karar:</strong> "flyer varyantları uygun" derseniz insan kapısı imzalanır ve
sıradaki aile önerilir; bulgu varsa kalem kalem yazın.</p>
</div>
<h2>Beş varyant + taban (tema: or-noir)</h2>
...
console.log(`galeri üretildi: docs/gt-designseed-flyer/cikti/galeri.html (${kb} KB, ${varyantKartlari.length}+${temaKartlari.length} kart)`);
