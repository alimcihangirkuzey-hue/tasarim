/* designSeed@trifold GÖRSEL GT galerisi (Canonical 4.4; journal 2026-07-26-designseed-trifold).
   Çalıştırma (repo kökünden — JSX automatic runtime için tsconfig ŞART):
     npx tsx --tsconfig packages/templates/tsconfig.json docs/gt-designseed-trifold/uret-galeri.mts
   Kartlar DIŞ YÜZ (pageIndex 0): kapak (flap) çerçevesi orada görünür. */
...
import { TEMPLATES, analyzeTrifold, type TrifoldFrame } from "@tezgah/templates";

const HERE = path.dirname(fileURLToPath(import.meta.url));
const ROOT = path.resolve(HERE, "..", "..");
const CIKTI = path.join(HERE, "cikti");

const ADLAR = ["Kebab", "Pizza Margherita", "Tacos XL", "Burger Maison", "Salade César", "Pide Fromage", "Lahmacun", "Soupe du Jour", "Baklava", "Dürüm Poulet", "Menu Enfant", "Assiette Mixte", "Café Gourmand", "Tiramisu", "Houmous"];
...
function musteri(): ClientDTO {
  return {
    ...
    currency: "EUR",
    menu_language: "fr",
    brandkit: BrandKitSchema.parse({ slogan_fr: "Le goût authentique", contact: { phone: "04 78 00 00 00" } }),
...
const doc = (params: Record<string, unknown> = {}, theme_id?: string): DocumentState =>
  DocumentStateSchema.parse({ template_id: "menu-trifold", params, theme_id });

function svgOf(d: DocumentState, c: ClientDTO): string {
  return renderToStaticMarkup(
    React.createElement(TEMPLATES["menu-trifold"].Component, { client: c, doc: d, mode: "print", pageIndex: 0 })
  );
}

/* Varyant temsilcileri — deterministik ilk-görülen taraması */
const c = musteri();
const temsilci = new Map<string, { seed: number; frame: TrifoldFrame }>();
for (let s = 1; s <= 200 && temsilci.size < 5; s++) {
  const frame = analyzeTrifold(c, doc({ designSeed: s })).frame;
...
const frameEtiket = (f: TrifoldFrame): string =>
  `kapak rx ${f.flapRx} · çizgi ${f.flapDash === null ? "düz" : `kesikli ${f.flapDash}`}`;
...
const varyantKartlari: Kart[] = [
  {
    ...
    alt: frameEtiket(analyzeTrifold(c, doc({})).frame),
    svg: svgOf(doc({}), c),
  },
...
const temaSeed = [...temsilci.values()].find((t) => t.frame.flapDash === null)?.seed ?? 1;
...
<title>designSeed@trifold — kapak çerçeve varyantları (görsel GT)</title>
<style>
${fontCss()}
...
.kart { margin: 0; width: min(420px, 100%); }
...
<h1>designSeed@trifold — kapak çerçeve varyantları (Canonical 4.4)</h1>
<div class="protokol">
<p><strong>Keşif kaydı:</strong> trifold'da baskıya çıkan tek tasarım çerçevesi DIŞ YÜZDEKİ
kapak (flap) çerçevesidir — ön paneldeki çerçeve yalnız edit modunda görünen kapak-foto yer
tutucusu olduğu için kapsam dışı bırakıldı; fold işaretleri üretim kılavuzudur.</p>
<p><strong>Yargı kalemleri (insan):</strong> kapak çerçeve dili panelle uyumlu mu? · beş
varyant baskı kalitesinde mi? · taban (seed 0) gözle de bozulmamış mı?</p>
<p><strong>Makinece zaten doğrulananlar</strong> (testli): taban birebir · tohum YALNIZ
çerçeveyi değiştirir (taşma muhasebesi + iç yüz sütun yerleşimi derin-eşit) · kapalı 5'li
küme · determinizm · tuzlar aileleri ayrıştırır.</p>
<p><strong>Karar:</strong> "trifold varyantları uygun" (ya da rapor protokolünce "devam et")
insan kapısını imzalar; bulgu varsa kalem kalem yazın.</p>
</div>
<h2>Beş varyant + taban (tema: or-noir, dış yüz)</h2>
...
console.log(`galeri üretildi: docs/gt-designseed-trifold/cikti/galeri.html (${kb} KB, ${varyantKartlari.length}+${temaKartlari.length} kart)`);
